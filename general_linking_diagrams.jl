...
function generalized_class_group(K::AbsSimpleNumField, S::Vector{AbsSimpleNumFieldOrderIdeal}, T::Vector{AbsSimpleNumFieldOrderIdeal};n_quo=0)
    if !isempty(intersect(S, T))
        error("S and T must be disjoint and at least one of them must be non-empty")
    end
    @assert all(order(q)==maximal_order(K) for q in union(S,T))
    m = prod(S; init = one(maximal_order(K))*maximal_order(K))
    Cl, Cl_map = ray_class_group(m; n_quo = n_quo)
    clST , _ = quo(Cl, sub(Cl, [Cl_map \ t for t in T])[1])
    return clST
end

function verify_Koch_set(K::AbsSimpleNumField, S::Vector{AbsSimpleNumFieldOrderIdeal}, T::Vector{AbsSimpleNumFieldOrderIdeal}, p::Int)
    if !isempty(intersect(S, T))
        error("S and T must be disjoint")
    end
    @assert all(order(q)==maximal_order(K) for q in union(S,T))
    ClST = generalized_class_group(K, S, T; n_quo = p)
    return is_trivial(ClST)&&is_trivial(kummer_group(K,S,p))
end
